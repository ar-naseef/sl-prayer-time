...
import type { PrayerTimesData, DistrictPrayerTimes, PrayerTime, MonthKey } from '@/data/prayerTimes';
...
function transformGroup(groupData: any): DistrictPrayerTimes {
...
export async function GET() {
  try {
    const jsonPath = path.join(process.cwd(), '..', 'one-of-scripts', 'prayer-times.json');
    const fileContents = await fs.readFile(jsonPath, 'utf-8');
    const rawData = JSON.parse(fileContents) as Record<string, any>;
    ...
    return NextResponse.json({ data: result, districts });
  } catch (error) {
